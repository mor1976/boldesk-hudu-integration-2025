export default async function handler(req, res) {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Hudu API error: ${response.status} ${response.statusText}`);
    }
    ...
    res.json({
      ...
      peopleLayout: peopleLayout ? {
        ...
        fieldsCount: peopleLayout.fields?.length || 0
      } : null,
      allLayouts: data.asset_layouts.map(l => ({ id: l.id, name: l.name }))
    });
